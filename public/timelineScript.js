...
function updateTimelineWithBark()
{
    ...
    barkCommentBtn.addEventListener('click', function()
    {
        ...
        {
            await commentFunction(barkID, commentContainerInputTextarea.value);
            document.getElementById('Site').removeChild(commentContainer);
        });
    ...
    const barkCommentCountNumber = document.createElement('p');
    barkCommentCountNumber.style.fontSize = '13px';
    barkCommentCountNumber.style.marginLeft = '5px';
    barkCommentCountNumber.textContent = '0';

    fs.collection(`barks`).doc(barkID).collection('comments').get()
    .then((snapshot) =>
    {
        barkCommentCountNumber.textContent = snapshot.size;
    });

    const barkViewCountIcon = document.createElement('div');
    barkViewCountIcon.className = 'viewsIcon';

    const barkViewCountNumber = document.createElement('p');
    barkViewCountNumber.style.fontSize = '13px';
    barkViewCountNumber.style.marginLeft = '5px';
    barkViewCountNumber.textContent = '0';
    ...
    barkBottomSpace.appendChild(barkCommentCountNumber);
    barkBottomSpace.appendChild(barkViewCountIcon);
    barkBottomSpace.appendChild(barkViewCountNumber);
